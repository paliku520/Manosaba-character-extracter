...
- **Hierarchy Viewer** — TreeView for viewing the character component hierarchy
- **Cache Reuse** — Extracted data cached in `temp/`, re-loading doesn't require re-unpacking
- **Multi-language** — Simplified Chinese / English / fiXmArge, auto-follows system, switchable in Settings
...
Click **Settings** on the left to configure: **Output Directory** (custom export location, remembered automatically), **Language**, **Check for Updates**, **Cleanup** (`temp/` cache or `output/` directory).
...
```
├── run.py             # Main entry
├── src/               # Core modules (loading, compositing, export, cache, i18n, etc.)
├── scripts/           # PyInstaller packaging scripts
├── output/            # Output directory (generated at runtime)
└── temp/              # Sprite cache (generated at runtime)
```

Tech stack: [UnityPy](https://github.com/K0lb3/UnityPy) (bundle parsing), Pillow (image processing), tkinter (GUI).
