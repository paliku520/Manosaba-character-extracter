...
- **层级结构** — TreeView 查看角色组件层级
- **缓存复用** — 已提取数据缓存到 `temp/`，重复加载无需重新解包
- **多语言** — 简体中文 / English / fiXmArge，自动跟随系统，可在设置中切换
...
点击左侧 **设置** 可配置：**输出目录**（自定义导出位置，自动记忆）、**语言**、**检查更新**、**清理**（`temp/` 缓存或 `output/` 目录）。
...
```
├── run.py             # 主程序入口
├── src/               # 核心模块（加载、合成、导出、缓存、i18n 等）
├── scripts/           # PyInstaller 打包脚本
├── output/            # 输出目录（程序生成）
└── temp/              # 精灵缓存（程序生成）
```

技术栈：[UnityPy](https://github.com/K0lb3/UnityPy)（bundle 解析）、Pillow（图像处理）、tkinter（GUI）。
